...
class CallingFormat(BaseModel):
    ...
    @staticmethod
    def generate_response(llm, tools, action_tool_mapping, user_prompt):
        ...
        response = llm.chat.completions.create(
            model="mistral:v0.3",
            messages=[
                {"role": "system", "content": f"DO NOT REPLY WITH ANYTHING NOT VALID JSON. You have access to these tools and may only use these tools and no others:\n{tool_descriptions_str}"},
                {"role": "system", "content": "To use a tool, provide a JSON object with the 'tool' key specifying the tool name and the 'tool_input' key(s) containing dictionary for ALL the required parameters from the valid tools list you were given."},
                {"role": "system", "content": "Your response should be only one single JSON array, where each element is a JSON object representing the tool selected for the action, for the entire user prompt."},
                ...
                {"role": "system", "content": "Example single JSON array format for multiple tools (formatting is important. Do not forget to use the quotations as required also): [{\"tool\": \"tool_name1\", \"tool_input\": {\"param1\": \"value1\", \"param2\": \"value2\"}}, {\"tool\": \"tool_name2\", \"tool_input\": {\"param3\": \"value3\"}}]"},
                {"role": "system", "content": "Tool input MUST be a dictionary for every individual tool_input, even if it's a single input. If a tool does not require any input parameters, provide an empty dictionary for 'tool_input', like this: {\"tool\": \"tool_name\", \"tool_input\": {}}"},
                {"role": "user", "content": f"Based on the mapping provided to you, '{action_tool_mapping}' and the user prompt {user_prompt}, please provide a single JSON array containing the appropriate tool usages. Fill in ALL the required parameters for each tool based on the prompt. If a required parameter is not provided in the prompt, make a reasonable assumption or leave it empty, but ensure that all required parameters are present in the JSON. If no specific tool is required for a part, do not include a JSON object for that part. Follow the example JSON format strictly and do not include any non-JSON text."},
            ],
            max_tokens=4096,
            top_p=0.5,
            temperature=0.5,
        ...
        try:
            ...
            resp = clean_response(response_text) 
...
class AgentCreator(BaseModel):
    ...
    def assign_agents(llm, agents, actions, max_retries=3):
        ...
        return action_agents, action_tool_mapping